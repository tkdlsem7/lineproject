...
const getEnv = () => {
  const viteEnv: AnyEnv = ((import.meta as any)?.env ?? {}) as AnyEnv; // Vite 호환
  const craEnv: AnyEnv =
    (typeof process !== "undefined" ? (process as any)?.env : {}) ?? {}; // CRA 호환

  // BASE: 절대호출 원할 때만 지정 (없으면 동일출처 + 프록시 사용)
  const API_BASE =
    String(viteEnv.VITE_API_BASE || craEnv.REACT_APP_API_BASE || "").replace(/\/+$/, "");
...
const buildUrl = (path: string) => {
  ...
  return `${API_BASE}${API_PREFIX}${p}`; // 예: http://127.0.0.1:8000 + /api + /dashboard/slots
};
